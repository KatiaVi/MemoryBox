...
#####################################
import os
os.system("set GOOGLE_APPLICATION_CREDENTIALS=My First Project-45ae84c60d7a.json")
...
def init(data):
    ...
    data.game = 0
    data.maxTime = int(input("write the number of seconds you expect to record for: "))
...
def runEmotionAnalyzer(data, timer):
	...
	dest = "data/%s/%s%d.mp3" % \
			(bestEmote, bestEmote, random.randint(0, 1000000))
...
def drawSplashScreen(canvas, data):
	...
	canvas.create_text(data.width/2, 3*data.height/4+30, text = "**please speak clearly", font = "Helvetica 10", fill="white")
